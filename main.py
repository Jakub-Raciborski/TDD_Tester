...
class FastMutator:
    ...
    def generate(self):
        for node in ast.walk(self.tree):
            ...
            if isinstance(node, ast.BoolOp):
                repl = {ast.And: ast.Or, ast.Or: ast.And}
                self._mutate(node.op, repl, "LCR")
    ...
    def _mutate(self, op, repl, label):
        for src, dst in repl.items():
            if isinstance(op, src):
                new = copy.deepcopy(self.tree)
                t = self._find(new, op)
...
def _calculate_coverage(code, asserts, logger_name):
    ...
    _execute(compiled, asserts, {logger_name: logger})
    total = len(hits) * 2
...
def ast_mutation_testing(code, asserts):
    ...
    for t, m in mutants:
        try:
            ...
            continue
        dead = any(not _execute(compiled, [a]) for a in asserts)
        if dead:
            killed += 1
        else:
            survived += 1
